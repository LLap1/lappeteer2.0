...
def read_size_prefixed_data() -> BytesIO:
    size_bytes: bytes = sys.stdin.buffer.read(8)
    if len(size_bytes) < 8:
        print("Error: Unable to read size", file=sys.stderr)
        sys.exit(1)
    
    size: int = struct.unpack('>Q', size_bytes)[0]
    print(f"Reading {size} bytes from stdin...", file=sys.stderr)
    
    data: BytesIO = BytesIO(sys.stdin.buffer.read(size))
    print(f"Read {len(data.getvalue())} bytes", file=sys.stderr)
...
def main() -> None:
    ...
    pptx_data: BytesIO = read_size_prefixed_data()
